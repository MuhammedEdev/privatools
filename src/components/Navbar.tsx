...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#090D16]/95 dark:bg-white/95 backdrop-blur-md border-b border-slate-800 dark:border-slate-200 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
              ...
              <Link
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-medium transition-all ${
                  isActive
                    ...
                    : "text-slate-300 dark:text-slate-600 hover:text-white dark:hover:text-slate-900 hover:bg-slate-900/60 dark:hover:bg-slate-200/60"
                }`}
              >
                {Icon && <Icon className={`w-3.5 h-3.5 ${isActive ? "text-emerald-400" : "text-slate-400 dark:text-slate-500"}`} />}
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Gerçek İkonlu Tema Değiştirme Butonu */}
          <button
            onClick={toggleTheme}
