...
export default function Base64Converter() {
  ...
  const handleProcess = (text: string, currentMode: "encode" | "decode") => {
    ...
    try {
      if (currentMode === "encode") {
        // Türkçe karakterler için btoa encoding güvenliği
        const encoded = btoa(
        ...
        // Türkçe karakterler için atob decoding güvenliği
      ...
      setError("Geçersiz Base64 formatı veya karakter dizilimi!");
      setOutput("");
    }
  };
  ...
  const toggleMode = (newMode: "encode" | "decode") => {
    setMode(newMode);
    // Mod değiştiğinde input ve output'u yer değiştirerek akışı hızlandıralım
    const temp = input;
    setInput(output);
    setOutput(temp);
    setError(null);
  ...
  return (
    <div className="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 text-white max-w-2xl mx-auto shadow-xl">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <h2 className="text-xl font-bold">Pro Base64 Encoder / Decoder</h2>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm text-zinc-400 mb-1">
            {mode === "encode" ? "Dönüştürülecek Metin (Plain Text)" : "Base64 Girdisi"}
          </label>
          <textarea
            rows={5}
            value={input}
            ...
            placeholder={mode === "encode" ? "Metni buraya girin..." : "Base64 kodunu buraya yapıştırın..."}
          />
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-1">
            <label className="block text-sm text-zinc-400">
              {mode === "encode" ? "Base64 Çıktısı" : "Çözülmüş Metin (Plain Text)"}
            </label>
            {output && (
              <button
